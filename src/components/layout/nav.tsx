    ...
    <>
      ...
      >
        <nav className="w-full">
          <div className="max-w-360 w-full mr-auto xl:w-[65%] 2xl:mr-auto flex items-center justify-between h-20 px-4 md:h-24 md:px-8 lg:px-10">
            ...
            <div className="hidden xl:flex">
              {ctaItem && <Button>Join waitlist</Button>}
            </div>
            <button
        ...
        <div
          className={cn(
            "fixed inset-0 z-20 top-0 pt-26 bg-[#F2F3FC] flex flex-col transition-transform duration-300 ease-in-out xl:hidden",
        ...
        >
          ...
          <ul className="flex flex-col">
            {regularLinks.map((item) => (
              <li key={item.label} className="border-b border-[#F7F9F8]">
                <Link
                  href={item.href}
                  onClick={() => setIsOpen(false)}
